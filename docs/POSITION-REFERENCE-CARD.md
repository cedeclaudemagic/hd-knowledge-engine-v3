# Electromagnetic Position Reference Card
...
**-4 SOURCE:** Where the wave originates. First principles derivation. The empty centre from which all structure emerges.

**-3 CAPACITANCE:** Stored potential in the void domain. The deep architecture beneath the visible surface. Color-Tone-Base, Lock-Key mechanism.

**-2 VOLTAGE:** The pressure differential that drives engagement. Creates the "why should I care?" The question that demands investigation.

**-1 GATE-OUT:** Where structure releases into formal expression. Mathematical proofs crossing from insight to articulation.
...
### Mathematician
```
-4 (Geometry) → -1 (Proof) → +3 (Architecture)
First principles → Formal proofs → HD derivations
```

### HD Practitioner
```
+1 (Wave) → +3 (Architecture) → +2 (Interpretations) → -3 (Depth)
Framework → HD structures → Any gate → Go deeper
```

### Complete Journey
```
-4 → -3 → -2 → -1 → +1 → +2 → +3 → +4
Source to Sink, void to form, theory to practice
```
...
| Position | Trigram | Binary | Element |
|----------|---------|--------|---------|
| -4 | Heaven | 111 | Pure yang |
| -3 | Lake | 110 | Yang storage |
| -2 | Fire | 101 | Yang flow |
| -1 | Wind | 011 | Transition out |
| +1 | Thunder | 100 | Transition in |
| +2 | Water | 010 | Yin flow |
| +3 | Mountain | 001 | Yin storage |
| +4 | Earth | 000 | Pure yin |
...
**"Where do I start?"** → Position +1 (The Wave)

**"Why does HD work?"** → Position +3 (The Architecture)

**"Show me the proofs"** → Position -1 (The Proof)

**"What's beneath the lines?"** → Position -3 (The Depth)

**"First principles derivation"** → Position -4 (The Geometry of Consciousness)
